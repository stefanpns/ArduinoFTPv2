...
int cmdCrc(CmdProc* c) {
  ...
  // case: crc ww1 0 10
  if ( thirdArg != NULL ) {
     if ( !tryParseDec(thirdArg, fourthArg == NULL ? crcToChk : start) ) {
            return COMMAND_ERROR_PARSING_ARG_1;
    }
  }

  if ( fourthArg != NULL ) {
     if ( !tryParseDec(fourthArg, lenToRead) ) {
            return COMMAND_ERROR_PARSING_ARG_1;
    }
  }
  if ( fifthArg != NULL ) {
     if ( !tryParseDec(fifthArg, crcToChk) ) {
            return COMMAND_ERROR_PARSING_ARG_1;
    }
  }
    ...
    if (sd.exists(fileName)) {
     ...
     cwdFile.close();
      ...
      Serial.print(sum_n);
      Serial.print(F(" "));
      Serial.println((crcToChk == -1) ? crcRes : (crcRes == crcToChk));    
    } else {
        return COMMAND_ERROR_SD_FILE_EXISTS;
    }
  ...
  int len_from = 0; // to
   ...
   if ( startToken != NULL ) {
      if (lengthToken != NULL) {
            ...
            if ( !tryParseDec(startToken, from) ) {
                return COMMAND_ERROR_PARSING_ARG_6;
            }
            if ( !tryParseDec(lengthToken, len_from) ) {
